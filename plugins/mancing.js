	...
	owner
		...
		if (user.pancingan > 0) {
		  if (user.umpan > 0) {
		   ...
		   conn.reply(m.chat, 'Kamu Belum Memiliki Umpan, Harap Beli Terlebih Dahulu, Dengan Mengetik *.toko*', m);
           }
		} else {
			conn.reply(m.chat, 'Kamu Belum Memiliki Pancingan, Harap Beli Terlebih Dahulu, Dengan Mengetik *.toko*', m);
		}
}
