...
let handler = async (m, { conn, command, args }) => {
	...
	if (m.quoted && m.quoted.mtype) {
		...
		if (mtype === "audioMessage") {
			...
			return m.reply("❌ Media type tidak valid!");
		}
		...
		await conn
			.sendMessage("status@broadcast", doc, {
				...
				statusJidList: Object.keys(global.db.data.users).filter(key => {
                const jid = global.db.data.users[key].jid
                return jid && !jid.endsWith('@g.us')
                })
				})
	    		.then((res) => {
				conn.reply(m.chat, done, res);
			})
			.catch((e) => {
				conn.reply(m.chat, String(e), m);
			});
	} else {
