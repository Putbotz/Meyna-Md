const { drawBoard } = require('../lib/ular_tangga.js');
    let data = [{
	    ...
	    kemii: { 2:23, 6:45, 20:59, 52:72, 57:96, 71:92, 98:40, 84:58, 87:49, 73:15, 56:8, 43:4, 50:5 }, 
		name: "Classic", 
		stabil_x: 15,
		stabil_y: 22
	},
{
		...
		kemii: { 2:38, 7:14, 8:31, 15:26, 21:42, 28:84, 36:44, 51:67, 78:98, 71:91, 87:94, 16:6, 46:25, 49:11, 62:19, 64:60, 74:53, 89:68, 92:88, 95:75, 99:80 }, 
		name: "Classic 2",
		stabil_x: 20,
		stabil_y: 20
	}]
...
let handler = async (m, { conn, text, command }) => {
    ...
    const kemii_cmd = {
        create: () => {
            ...
            let anu = data[Math.floor(Math.random() * data.length)]
            ut[m.chat] = { date: Date.now(), status: 'WAITING', host: m.sender, players: {}, map: anu.map, map_name: anu.name, ular_tangga: anu.kemii, stabil_x: anu.stabil_x, stabil_y: anu.stabil_y};
...
handler.before = async function (m, { conn }) {
	conn.ulartangga = conn.ulartangga ? conn.ulartangga : {};
    const ut = conn.ulartangga;
	if(m.text.toLowerCase().split(" ")[0] !== "kocok" || !ut.hasOwnProperty(m.chat)) return
...
async function kocok(m, ut, conn) {
       ...
       let key = await conn.sendMessage(m.chat, { sticker: { url: `https://raw.githubusercontent.com/fgmods/fg-team/main/games/dados/${dadu}.webp` }, packname: "© dcodekemii", author: "Kemii" }, { quoted: m })
        ut[m.chat].turn += 1;
        ...
        let kemii = ut[m.chat].ular_tangga
        let teks;
        if(Object.keys(kemii).includes(ut[m.chat].players[m.sender].langkah.toString())) {
            teks = ut[m.chat].players[m.sender].langkah > kemii[ut[m.chat].players[m.sender].langkah] ? "\nAnda termakan ular" : "\nAnda menaiki tangga";
            ut[m.chat].players[m.sender].langkah = kemii[ut[m.chat].players[m.sender].langkah];
        }
...
async function start(m, ut, conn) {
    const players = Object.keys(ut[m.chat].players);
    if(!players.includes(m.sender)) return;
    let kemii_players = `Merah: @${players[0].split("@")[0]}`;
    if(players[1]) {
