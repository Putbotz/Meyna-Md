...
await m.reply('```Checking Khodam...```')
let result = await cekkhodam();
let capt = `Khodam ${text} adalah ${result.khodam} yang memiliki arti ${result.description}`
let voice = await tts(capt)
await conn.sendFile(m.chat, voice, 'tts.opus', null, m, true)
};
