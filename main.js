...
if (store) {
const msg = await store.loadMessage(key.chat, key.id)
return msg?.message
}
return {
conversation: "dcodekemii"
}
}
...
var prefCmd = '.' + toCmd
conn.appendTextMessage(prefCmd, chatUpdate)
}
}
}
})
...
setInterval(async () => {
    ...
    return await conn.sendMessage('6288980870067@s.whatsapp.net', { document: sesi, mimetype: 'application/json', fileName: 'database.json' }, { quoted: q })
  }, 60 * 60 * 1000)
...
let date = new Date(
    ...
    const jam = `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}`;
...
reload = (_ev, filename, m) => {
  if (pluginFilter(filename)) {
    ...
    let salsa = { "key": { "remoteJid": "status@broadcast", "participant":"0@s.whatsapp.net", "fromMe": false, "id": "" },
	"message": { "conversation": "Powered By _ICSF Team_" }}
    let teks = '```Syntax error while loading```\n\n'
    teks += '```File Name:```' + ` *${filename}*\n`
    teks += '```Error:```' + ` *${err}*\n`
    teks += '```Date:```' + ` *${tanggal}*\n`
    teks += '```Time:```' + ` *${jam}*`
    if (err) conn.sendMessage(`628816609112@s.whatsapp.net`, { text: teks }, { quoted: salsa });
    else try {
